...
# wkhtmltopdf is required for the PDF-export sanity check the v0.5.0 panels
# touch. Skip on local dev (it's a slow 80 MB install) by setting SKIP_WKHTMLTOPDF=1.
if [ -z "${SKIP_WKHTMLTOPDF:-}" ] && ! command -v wkhtmltopdf >/dev/null 2>&1; then
	sudo apt-get install -qy --no-install-recommends wkhtmltopdf
fi
